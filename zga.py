...
def parse_args():
	...
	# General options
	parser.add_argument("-s", "--step", help="Starting step of pipeline",default="qc", choices=["qc", "processing", "assembling", "annotation", "checking"])
	...
	parser.add_argument("-m", "--memory-limit", type=int, help="Memory limit in GB")
	parser.add_argument("--genus", default="Unknown", help="")
	parser.add_argument("--species", default="sp.", help="")
	parser.add_argument("--strain", help="")
	...
	# Input
	parser.add_argument("-1", "--pe-1", help="FASTQ file of first (left) short reads")
	parser.add_argument("-2", "--pe-2", help="FASTQ file of second (right) short reads")
	...
	parser.add_argument("--merge-with", default="bbmerge", choices=["bbmerge", "seqprep"],
		help="Tool for merging overlapping contigs")
	...
	parser.add_argument('--linear_seqs', default=0, help="Expected number of linear sequences")
	

	# Annotation
	parser.add_argument("-g", "--genome", help="Genome assembly when starting from annotation.")
	parser.add_argument("--locus-tag", help="Locus tag prefix. If not provided random will be generated.")
	...
	# CheckM
	parser.add_argument("--checkm_mode", default="taxonomy_wf", choices=['taxonomy_wf', 'lineage_wf'],
...
def merge_bb(args, reads, readdir):
	...
	cmd = ['bbmerge.sh', f"in1={reads['pe_1']}", f"in2={reads['pe_2']}",
		f"outu1={nmr1}", f"outu2={nmr2}", f"out={merged}"]
	if bb_trim:
...
def assemble(args, reads):
	...
	if args.assembler == 'spades':
		...
		if 'pe_1' not in reads.keys() or 'pe_2' not in reads.keys():
			logger.error("Missing paired-end reads for \'Unicycler\'.")
			raise Exception("Extermal software error")

		cmd = ['unicycler', '-1', reads['pe_1'], '-2', reads['pe_2'], '-o', aslydir,
			'-t', str(args.threads)]
		if args.no_correct:
			cmd += ['--no_correct']
		if 'merged' in reads.keys():
			...
			cmd += ['-s', reads['single']]
		if 'nanopore' in reads.keys() and 'pacbio' in reads.keys():
			logger.error("Imposible to use both pacbio and nanopore reads in unicycler.")
			logger.error("Please select only one file with long reads.")
			raise Exception("Extermal software error")
		elif 'nanopore' in reads.keys():
...
def locus_tag_gen(genome):
	...
	with open(genome, 'rb') as genomefile:
		#locus_tag = ''.join(list( map(lambda x: chr(ord(x) + 23), hashlib.md5(genomefile.read()).hexdigest()[0:6] ) ))
...
def run_checkm(args, assembly):
	...
	if args.checkm_mode == "taxonomy_wf":
		...
		cmd = ['checkm', 'taxonomy_wf', '-f', checkm_outfile, '-x', 'fasta', '-t', str(args.threads)]
		...
		cmd = ['checkm', 'lineage_wf', '-f', checkm_outfile, '-t', str(args.threads)]
...
def main():
	...
	steps = {"qc":1, "processing":2, "assembling":3, "annotation":5, "checking":4}
